...
class CarModelPlatformCRUD(BaseCRUD[CarModelPlatform, CarModelPlatformCreate, CarModelPlatformUpdate]):
    ...
    async def get_by_platforms_and_car(self, platforms_ids: list[str], car_model_id: str) -> list[CarModelPlatform]:
        stmt = (
    ...
    async def get_by_car(self, car_model_id: str) -> list[CarModelPlatform]:
        stmt = (
            select(CarModelPlatform)
            .where(CarModelPlatform.car_model_id == car_model_id)
            .options(joinedload(CarModelPlatform.platform))
        )
        result = await self.db.execute(stmt)
        return list(result.unique().scalars().all())
